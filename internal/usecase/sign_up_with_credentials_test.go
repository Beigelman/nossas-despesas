...
func TestSignUpWithCredentials(t *testing.T) {
	...
	auth, _ := entity.NewCredentialAuth(entity.CredentialsAuthParams{
		ID:       entity.AuthID{Value: 1},
		Email:    "test@email.com",
		Password: "12345678",
	})
	...
	t.Run("should return error with authRepo fails", func(t *testing.T) {
		authRepo.EXPECT().GetByEmail(ctx, "test@email.com", entity.AuthTypes.Credentials).Return(nil, errors.New("test error")).Once()
	...
	t.Run("should return error if auth method already exists", func(t *testing.T) {
		authRepo.EXPECT().GetByEmail(ctx, "test@email.com", entity.AuthTypes.Credentials).Return(auth, nil).Once()
	...
	t.Run("should return error if password and confirm password does not match", func(t *testing.T) {
		authRepo.EXPECT().GetByEmail(ctx, "test@email.com", entity.AuthTypes.Credentials).Return(nil, nil).Once()
	...
	t.Run("should return error if authRepo fails", func(t *testing.T) {
		...
		authRepo.EXPECT().GetNextID().Return(entity.AuthID{Value: 1}).Once()
