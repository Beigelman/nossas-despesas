...
func TestInviteUserToGroup(t *testing.T) {
	...
	for i := 0; i < 4; i++ {
		invites = append(invites, *entity.NewGroupInvite(entity.GroupInviteParams{
			ID:        entity.GroupInviteID{Value: i},
			GroupID:   group.ID,
			Token:     "testToken",
			Email:     "john@email.com",
			ExpiresAt: time.Now(),
	...
	t.Run("should return error if groupInviteRepo fails", func(t *testing.T) {
		...
		groupInviteRepo.EXPECT().GetNextID().Return(entity.GroupInviteID{Value: 1}).Once()
