...
func TestCreateExpense(t *testing.T) {
	...
	createExpense := NewCreateExpense(expenseRepo, userRepo, groupRepo, categoryRepo, incomeRepo)
	...
	t.Run("should return error if payer's and receiver's group does not match", func(t *testing.T) {
		userRepo.EXPECT().GetByID(ctx, payer.ID).Return(payer, nil).Once()
		userRepo.EXPECT().GetByID(ctx, receiver.ID).Return(receiver, nil).Once()
		groupRepo.EXPECT().GetByID(ctx, group.GroupID{Value: 3}).Return(group.NewGroup(group.GroupParams{ID: group.GroupID{Value: 3}}), nil).Once()

		p := CreateExpenseParams{
			PayerID:     payer.ID,
			ReceiverID:  receiver.ID,
			GroupID:     group.GroupID{Value: 3},
			CategoryID:  category.ID,
			SplitType:   "equal",
			Name:        "name",
			Amount:      100,
	...
	t.Run("happy path with equal split ratio", func(t *testing.T) {
		...
		assert.Equal(t, expense.NewEqualSplitRatio(), expense.SplitRatio)
