...
func TestToEntity(t *testing.T) {
	...
	assert.Equal(t, entity.CategoryGroupID{Value: 1}, category.ID)
	...
	category = toEntity(model)
	assert.NotNil(t, category)
	assert.Equal(t, entity.CategoryGroupID{Value: 2}, category.ID)
...
func TestToModel(t *testing.T) {
	...
	category = &entity.CategoryGroup{
		Entity: ddd.Entity[entity.CategoryGroupID]{
			ID:        entity.CategoryGroupID{Value: 2},
			CreatedAt: time.Now(),
			UpdatedAt: time.Now(),
			DeletedAt: nil,
			Version:   2,
